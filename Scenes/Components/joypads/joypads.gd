...
func _input(event: InputEvent) -> void:
	if event is InputEventJoypadButton and event.button_index == 6 and event.device != cur_joy:
		joypad_number.value = event.device
		Input.start_joy_vibration(event.device, 1.0, 1.0, 0.25)
